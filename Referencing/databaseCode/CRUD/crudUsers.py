...
def getUserById(userId, dbName=DB_NAME_DEFAULT):
    ...
    if db is None:
        return None
    try:
        return db.users.find_one({"_id": toObjectId(userId)})
    finally:
        closeConnection(client)
...
def updateUserById(userId, updateFields, dbName=DB_NAME_DEFAULT):
    ...
    if db is None:
        return 0
    try:
        res = db.users.update_one({"_id": toObjectId(userId)}, {"$set": updateFields})
...
def deleteUserById(userId, dbName=DB_NAME_DEFAULT):
    ...
    if db is None:
        return 0
    try:
        res = db.users.delete_one({"_id": toObjectId(userId)})
