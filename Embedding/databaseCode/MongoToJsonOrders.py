...
def exportOrdersToJson(batchSize=50, collectionName='orders', outputDir=None, dbName=None):
	if outputDir is None:
		outputDir = os.path.join(os.path.dirname(__file__), 'JsonOrders')
	...
	try:
		ordersCollection = db[collectionName]
		...
		for doc in cursor:
			...
			if len(batch) >= batchSize:
				filePath = os.path.join(outputDir, f'orders_{fileIndex}.json')
				with open(filePath, 'w', encoding='utf-8') as f:
					json.dump(batch, f, ensure_ascii=False, indent=2)
				print(f'Wrote {len(batch)} orders to {filePath}')
				fileIndex += 1
				batch = []
