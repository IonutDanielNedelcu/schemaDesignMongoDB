...
def populateVendors(db=None, confirm=True):
    ...
    try:
        if db is None:
            # import local connection helper
            from connection import connectToMongoDB, closeConnection
            client, db = connectToMongoDB()
            closeClient = True

        if db is None:
            print("No database available. Aborting.")
            return

        coll = db.get_collection("vendors")

        if confirm:
            print(f"Inserting {len(vendors)} vendor documents into {db.name}.vendors")

        res = coll.insert_many(vendors)
        print(f"Inserted {len(res.inserted_ids)} documents into vendors collection.")

    except Exception as e:
        print("Failed to populate vendors:", e)

    finally:
        if closeClient and client:
            try:
